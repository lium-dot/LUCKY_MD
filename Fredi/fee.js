...
module.exports = toxic = async (client, m, chatUpdate, store) => {
    try {
        ...
        if (!settings) {
            console.error("Fee-Xmd: Settings not found, cannot proceed!");
            return;
        }
        ...
        const filePath = require('path').resolve(__dirname, '../fredi.jpg');
        ...
        try {
            ...
            m.metadata = m.isGroup ? await client.groupMetadata(m.chat).catch(e => {
                console.error("Fee-Xmd: Group metadata fetch error:", e);
                return {};
            }) : {};
            ...
            console.error("Fee-Xmd: Error fetching group metadata:", error);
            m.metadata = {};
            m.isAdmin = false;
            m.isBotAdmin = false;
        }
        ...
        console.error('Fee-Xmd Error:', util.format(err));
    }
    ...
    process.on('uncaughtException', function (err) {
        ...
        console.error('Fee-Xmd Caught exception:', err);
    });
};